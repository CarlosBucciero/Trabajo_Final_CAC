...
public class SociosDao 
{
    ...
    public List<Socios> listarSocios()
    {
	   ...
	   rs=ps.executeQuery();
           ...
           while(rs.next())
           {
               ...
               Socios s1=new Socios(id,nombre,apellido,localidad,fecha,email,telefono,activo);
               lista.add(s1);                
    ...
    public Socios mostrarSocio(int _id)
    {
        ...
        try
        {
            ...
            while(rs.next())
            {
                ...
                String telefono=rs.getString("telefono");
                boolean activo=rs.getBoolean("activo");                 
                s1=new Socios(id,nombre,apellido,localidad,fnac,email,telefono,activo);
            }
    ...
    public boolean InsertSocios(Socios s1)
    {
        ...
        try
        {
            ps=conexion.prepareStatement("insert into socios (nombre,apellido,localidad,fnac,email,telefono,activo) values (?,?,?,?,?,?,?,?)");
    ...
    public boolean ActualizarSocios(Socios s1)
    {
        ...
        try
        {
            ps=conexion.prepareStatement("update socios set nombre=?,apellido=?,localidad=?,fnac=?,email=?,telefono=?,activo=? where idSocio=?");
            ps.setString(1, s1.getNombre());
            ps.setString(2, s1.getApellido());
